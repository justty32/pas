...
| 4 NPC outpost | 原版 `Settlement`（lazy 守軍 + 白嫖 visit/trade/attack）；Faction Territories 認 outpost | ⚠️自寫據點＝**繼承 `Settlement` + 抽象產出 comp**（VOE 是現成但**錯工具**：綁玩家+常駐 pawn 太重、且 `Map` 恆 null **永不能被訪問/襲擊**，R2 §2.5）。使用者要保留「可訪問/襲擊」後路→「抽象 vs 可造訪」是同物件有無 map 兩態，非兩型別 | 中（**現成不適用例外**） |
