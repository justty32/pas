...
*   **FactionManager**: 全局單例，管理所有現存勢力。
    *   ℹ️ 核對 2026-06-01：慣用存取方式是 `Faction.OfPlayer`、`Faction.OfMechanoids`、`Faction.OfInsects`（`Faction` 類別上的靜態屬性），內部才委派給 `Find.FactionManager`。請勿直接寫 `Find.FactionManager.OfPlayer` 或把它們當成屬於 `FactionManager` 的公開 API。
