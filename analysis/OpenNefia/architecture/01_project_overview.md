...
| 技術 | 用途 |
|------|------|
| **C# / .NET 8.0** | 主要開發語言與執行環境（舊分析誤記為 6.0） |
| **LÖVE 2D (via Love2dCS)** | 圖形渲染後端（2D 遊戲框架，自訂 fork） |
| **NLua** | Lua 腳本整合（本地化、部分遊戲邏輯） |
| **YamlDotNet** | 解析 YAML 格式的原型資料檔案 |
| **CSharpRepl** | 遊戲內 C# REPL 開發者主控台 |
| **xUnit** | 單元測試框架 |
| **MSBuild** | 建構系統 |
| **XamlX / XamlInjectors** | 編譯期 XAML→IL 注入（Avalonia 同款技術，供 Wisp UI 框架使用） |
| **Lib.Harmony** | .NET 執行期熱補丁（HarmonyX，用於遊戲邏輯 patch） |
| **SixLabors.ImageSharp** | 純 .NET 影像處理（圖片載入、像素操作） |
| **Serilog** | 結構化日誌後端 |
| **Pidgin** | Parser combinator 函式庫（用於語法解析） |
| **Nett** | TOML 設定檔解析 |
| **Melanchall.DryWetMidi** | MIDI 音樂播放 |
| **NuGet.* (6.8)** | 執行期 NuGet 套件管理（動態載入 Mod 依賴） |
| **TextCopy** | 跨平台剪貼簿存取 |
| **NativeFileDialogSharp** | 原生作業系統檔案對話框 |
...
│   ├── DebugView/Controls/       ← XAML 控件範例（*.xaml + *.xaml.cs code-behind）
│   └── [各子系統目錄]/
│
├── OpenNefia.EntryPoint/
│   └── Program.cs                ← Main() 入口
│
├── OpenNefia.XamlInjectors/      ← 編譯期 XAML→IL 注入工具（MSBuild Task）
├── OpenNefia.XamlNameGenerator/  ← 編譯期 XAML 名稱屬性生成器
├── OpenNefia.YAMLValidator/      ← 原型 YAML schema 驗證工具
├── Mods/
│   └── OpenNefia.LecchoTorte/   ← 額外內容 Mod（放置於 Mods/ 目錄）
├── Support/                      ← 原版遊戲資源（圖片、音樂等）
├── Resources/                    ← 引擎內建資源
├── docs/                         ← DocFX 文件設定
├── build.sh / build.ps1          ← 建構腳本
├── stylua.toml                   ← Lua 程式碼格式設定
├── TaskList.xml                  ← 開發 TODO 追蹤（xml 格式）
