...
def _make_shape_mask(
    ...
    if shape == "island":
        ...
        for y in range(height):
            for x in range(width):
                d = ((x - cx) ** 2 + (y - cy) ** 2) ** 0.5
                mask[y][x] = max(0.0, 1.0 - (d / land_r) ** 1.3)
        return mask
        ...
        for y in range(height):
            for x in range(width):
                ...
                mask[y][x] = max(0.0, 1.0 - (d_from_ring / ring_w) ** 1.5)
        return mask
...
def generate_heightmap(
            ...
            for x in range(width):
                ...
                if _use_ridge:
                    ...
                    if _gate is not None:
                        local_w = ridge_weight * _gate[y][x]
                    else:
                    ...
                    if local_w > 0.0:
                        ...
                        if _ridge_carry is not None:
                            fold *= _ridge_carry[y][x]
                            # clamp 防止 carry 爆衝；gain=2 在 fold≈0.5 時讓 carry≈1 穩定
                            _ridge_carry[y][x] = min(1.0, fold * ridge_multifractal_gain)
    ...
    if shape is not None:
        ...
        for y in range(height):
            for x in range(width):
                m = mask[y][x]
                target = m * (sl + grid[y][x] * (1.0 - sl))
                grid[y][x] = grid[y][x] * (1.0 - s) + target * s
