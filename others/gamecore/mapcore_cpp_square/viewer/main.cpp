...
std::string run_generate_noise(viewer::EditorState& s) {
    ...
    p.shape           = kShapeApi[std::clamp(s.noise_shape, 0, static_cast<int>(IM_ARRAYSIZE(kShapeApi)) - 1)];
...
int main()
{
    ...
    InitWindow(kWinW, kWinH, "mapcore_cpp viewer — Square Grid");
    SetTargetFPS(60);
    rlImGuiSetup(true);
    ...
    auto center_camera = [](Camera2D& cam, const viewer::EditorState& s) {
        const Vector2 cmax = viewer::canvas_pixel_size(s.width(), s.height(), s.cell_size);
    ...
    while (!WindowShouldClose())
    {
        ...
        const viewer::GridCoord under_mouse =
            viewer::pixel_to_grid(mouse_world.x, mouse_world.y, state.cell_size);
            ...
            if (now - last_brush_t >= interval) {
                ...
                switch (state.current_tool) {
                    case viewer::Tool::Raise:
                        viewer::apply_brush(state, under_mouse.x, under_mouse.y,  strength);
                        break;
        ...
        // ── 鍵盤平移 ───────────────────────────────────────────────────
        if (!ui_wants_kb) {
            const float step = std::max(20.0f, state.cell_size * 2.0f) / camera.zoom;
        ...
        if (in_bounds) {
            ...
            const auto disk = viewer::grid_disk(under_mouse.x, under_mouse.y, radius - 1);
            for (const auto& g : disk) {
                if (!state.in_bounds(g.x, g.y)) continue;
                const float px = static_cast<float>(g.x) * state.cell_size;
                const float py = static_cast<float>(g.y) * state.cell_size;
                DrawRectangleLinesEx({px, py, state.cell_size, state.cell_size}, 1.2f, Color{255, 220, 80, 110});
            }
            const float px = static_cast<float>(under_mouse.x) * state.cell_size;
            const float py = static_cast<float>(under_mouse.y) * state.cell_size;
            DrawRectangleLinesEx({px, py, state.cell_size, state.cell_size}, 2.5f, Color{255, 220, 80, 255});
        }
        EndMode2D();

        DrawText("Square Grid World Sculptor", 16, 16, 20, RAYWHITE);
        DrawFPS(16, kWinH - 28);
