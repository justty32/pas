...
TEST_CASE("NPC AI — 警覺且鄰接時攻擊英雄（核心戰鬥邏輯）") {
    ...
    em.emplace<opennefia::HeroComponent>(hero);  // 正向辨識標記
...
TEST_CASE("HeroComponent — 空 tag 序列化 round-trip 後仍可辨識英雄") {
    ...
    auto hero = em.create();
    em.emplace<opennefia::MetaDataComponent>(hero, opennefia::MetaDataComponent{"hero", true});
    em.emplace<opennefia::SpatialComponent>(hero, opennefia::SpatialComponent{4, 7, entt::null});
    em.emplace<opennefia::HealthComponent>(hero, 20, 20);
    em.emplace<opennefia::HeroComponent>(hero);

    // 另建一個非英雄實體，確保不是「碰巧只有一個」
    auto npc = em.create();
    em.emplace<opennefia::MetaDataComponent>(npc, opennefia::MetaDataComponent{"putit", true});
    em.emplace<opennefia::SpatialComponent>(npc, opennefia::SpatialComponent{1, 1, entt::null});
    em.emplace<opennefia::NpcAiComponent>(npc, opennefia::NpcAiComponent{});

    std::ostringstream oss;
    opennefia::serialize::save(em.registry(), oss);
    em = opennefia::EntityManager{};
    std::istringstream iss{oss.str()};
    opennefia::serialize::load(em.registry(), iss);

    auto& reg = em.registry();
    int hero_count = 0;
    entt::entity found = entt::null;
    for (auto e : reg.view<opennefia::HeroComponent>()) { ++hero_count; found = e; }
    bool found_valid = (found != entt::null);  // 先求值，避免 doctest 分解 operator!= 歧義
    CHECK(hero_count == 1);
    CHECK(found_valid);
    CHECK(reg.get<opennefia::SpatialComponent>(found).x == 4);
    CHECK(reg.get<opennefia::SpatialComponent>(found).y == 7);
}
