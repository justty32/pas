...
- 2026-06-02：**在地化系統（核心回歸③）**。新增 `core/locale/LocaleRegistry`（`load`/`get`/`get+fallback`/`get+{var}替換`/`has`；找不到 key 回傳 key 本身，前向相容）；`data/locale/zh-TW.yaml`（NPC 名稱 3 條 + 道具名稱 1 條 + 訊息模板 5 條）；`ServiceContext` 加 `locale` 成員；`opennefia_world_gd.cpp` 的 `npc_died`/`hero_bumped_npc`/`item_picked_up` signal 改發送在地化名稱（普提特/戰士/蝙蝠/治癒藥草）。`test_locale.cpp` 6 案例 11 assertions。ctest **52 cases / 179 assertions 全綠**，VERIFY PASSED。
- 2026-06-02：**CVar 設定系統（核心回歸②）**。新增 `core/cvar/CvarRegistry`（`reg<T>/get<T>/set<T>/reset/save/load`，支援 int/float/bool/string，YAML 存讀）；`ServiceContext` 加 `cvars` 公開成員；`OpenNefiaWorld::init_cvars()` 登錄 6 個遊戲參數（地圖尺寸/NPC上限/FOV半徑/物品機率）；`setup_map()`/`recompute_fov()` 的硬編數字改從 CVar 讀取。`test_cvar.cpp` 6 案例 22 assertions。ctest **46 cases / 168 assertions 全綠**，VERIFY PASSED。
- 2026-06-02：**原型 YAML 資料化（F7 / 核心回歸①）**。建 `data/game_prototypes.yaml`（6 原型：`BaseNpc`/`Putit`/`Warrior`/`Bat`/`HealthPotion`/`Hero`，繼承關係已解析）。`CombatStatsComponent` 加 `base_hp`/`hp_per_floor`，`ItemComponent` 加 `value_per_floor`（存讀檔格式同步更新）。`OpenNefiaWorld` 加 `PrototypeManager pm_`，新增 `init_prototypes()`（5 個 ComponentLoader：NpcAi/CombatStats/Health/Item/Hero）；`setup_map()` 改用 `pm_.spawn()` 取代全部硬編生成，層數縮放改成 `base + (floor-1)*scale` 後置套用。CMakeLists 加 `OPENNEFIA_DATA_DIR` define。ctest **40 cases 全綠**，headless `VERIFY PASSED`。
</content>
