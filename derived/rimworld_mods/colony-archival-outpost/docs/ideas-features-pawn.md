...
- ⚠ **技術現實／待釐清（與 HP/技能不同，更棘手）**：
  ...
  2. **佔位符孩童會不會長大？**：**查證結論（2026-06-10，VOE Outpost.Tick/TickInterval 反編譯）：否。**
     VOE `Tick()` 只呼叫 `WorldObject.Tick()` 與 `SatisfyNeeds()`；`TickInterval()` 只處理生產計時與基本需求。
     **occupant pawn 完全不被 tick** ——年齡不推進、成長時刻永不觸發。
     → 本功能在 VOE 架構下**無法直接實作**，除非在我方 `Outpost_Sampled.Produce()` 或 `Tick()` 裡手動推進
     孩童年齡（非 trivial，且可能與其他 mod 衝突）。暫時封鎖，存查。
